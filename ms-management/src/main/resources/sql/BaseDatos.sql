...
-- TABLE PERSON
CREATE TABLE IF NOT EXISTS management.persons
(
...
-- INDEXES FOR PERSON
CREATE INDEX person_name_idx ON management.persons (name);
CREATE INDEX person_identification_idx ON management.persons (identification);
...
(
    ...
    CONSTRAINT client_person_fk FOREIGN KEY (id) REFERENCES management.persons (id) ON DELETE CASCADE
);
...
(
    ...
    status          BOOLEAN        NOT NULL DEFAULT TRUE,
    client_id       int4,
    CONSTRAINT account_client_fk FOREIGN KEY (client_id) REFERENCES management.clients ON DELETE CASCADE
);
